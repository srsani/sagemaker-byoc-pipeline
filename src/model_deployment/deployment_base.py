...
def upload_data_s3():
    ...
    try:
        response = s3_client.upload_file('/opt/ml/processing/model/model.tar.gz',
                                         bucket_name,
                                         f'model/s3_folder_prefix/model.tar.gz')
    except ClientError as e:
        raise e
    return response
...
if __name__ == "__main__":
    ...
    upload_data_s3()
    logger.info('='*20)
    logger.info('upload model tar file to model/s3_folder_prefix/')
    ...
    endpoint_name = endpoint_creation(endpoint_config_name=endpoint_config_name,
                                      client=client,
                                      endpoint_name='s3_folder_prefix')
